...
int OuraGetSize (TOuras *oura, TSOuras stoixeio){
	int ipolipo;
	ipolipo=oura->piso-oura->embros;
	return ipolipo;
}
int GetCountIn(TOuras *oura, TSOuras stoixeia){
    oura->CountIn=oura->CountIn+1;
	return oura->CountIn;
}
int GetCountOut(TOuras *oura, TSOuras stoixeia){
    oura->CountOut=oura->CountOut+1;
